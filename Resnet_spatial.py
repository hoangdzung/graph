...
class Resnet50():
    ...
    def conv_layer(input_layer, data, layer_name, block, strides = [1,1,1,1], padding = 'VALID', use_relu = False):
        with tf.variable_scope(layer_name):
            ...
            x = tf.nn.conv2d(input_layer, filter=W, strides=strides, padding=padding, name='conv')
    ...
    def _build_model(self):
        ...
        with tf.device("/gpu:0"):
            with tf.variable_scope('stage1'):
                res = self.conv_layer(self.input_matrix, data_h5, 'conv1', block = True, strides=[1, 2, 2, 1])
                res = self.batch_norm_layer(res, data_h5, 'bn_conv1', block = True)
                res = tf.nn.relu(res)
                res = tf.nn.max_pool(res, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID', name='pool_conv1')

            with tf.variable_scope('stage2'):
                res = self.conv_block(input_layer=res, stage='2a', data=data_h5, block = True, strides=[1, 1, 1, 1])
                res = self.identity_block(input_layer=res, stage='2b', block = True, data=data_h5)
                res = self.identity_block(input_layer=res, stage='2c', block = True, data=data_h5)

            with tf.variable_scope('stage3'):
                res = self.conv_block(input_layer=res, stage='3a', block = True, data=data_h5)
                res = self.identity_block(input_layer=res, stage='3b', block = True, data=data_h5)
                res = self.identity_block(input_layer=res, stage='3c', block = True, data=data_h5)
                res = self.identity_block(input_layer=res, stage='3d', block = True, data=data_h5)

            with tf.variable_scope('stage4'):
                res = self.conv_block(input_layer=res, stage='4a', block = True, data=data_h5)
                res = self.identity_block(input_layer=res, stage='4b', block = True, data=data_h5)
