    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement =True)) as sess:
        ...
        for epoch in range(n_epoch):
            ...
            for i in range(model.queue.num_batch_train):
                ...
                loss, _, _,_,acc  = sess.run([model.loss, model.optimizer, model.spatial_centers_update_op, model.temporal_centers_update_op, model.accuracy], feed_dict = feed_dict)
                
                # Print results after each batch
                print("{:3d}/{:3d}: acc {:0.5f}, loss {:3.5f}, total_loss {:5.5f}".format(batch, model.queue.num_batch_train, acc, loss, total_train_loss),end='\r')
		step +=1
            ...
            for i in range(model.queue.num_batch_test):
                ...
                # Print results after each batch
                print("{:3d}/{:3d}: acc {:0.5f}, loss {:3.5f}, total_loss {:5.5f}".format(batch, model.queue.num_batch_test, acc, loss, total_test_loss),end='\r')
...
def main():
    ...
    model = Multi_stream.Multi_stream_model(queue_data=dataset, h5_path=sys.argv[3], learning_rate=0.1,momentum=0.8) 
